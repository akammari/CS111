public class LeasingCost {
    ...
    public static Vehicle createVehicle(String description){
       String car = getinfo("name:", description);
       String FuelType = getinfo("type:", description);
       Fuel fuel;


       if (FuelType.equals("gas")) {
           Double usage = Double.parseDouble(getinfo("mile/unit:", description));
           fuel = new Fuel(usage);
       } else {
           Double usage = Double.parseDouble(getinfo("mile/unit:", description));
           Double charger = Double.parseDouble(getinfo("charger:", description));
           fuel = new Fuel(usage, charger);
       }
 
       double dueAtSigning = Double.parseDouble(getinfo("due:", description));

       int leaseLength=Integer.parseInt(getinfo("length:", description));

       double monthlyCost=Double.parseDouble(getinfo("monthly:", description));

       int mileageAllowance = Integer.parseInt(getinfo("allowance:", description));

       Lease lease = new Lease(dueAtSigning, leaseLength, monthlyCost, mileageAllowance);

       Vehicle newCar = new Vehicle(car, fuel, lease);


        return newCar;
    }
 
    public static String getinfo(String s, String description) {
        String a = description.substring(description.indexOf(s)+String.valueOf(s).length(),description.indexOf(';',description.indexOf(s)));
        return a;
    }
    ...
    public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
       ...
       for (Vehicle v : vehicles) {
            int numofMonth = v.getLease().getLeaseLength();
            double co2;
            double usage = v.getFuel().getUsage();
            double mileageAllowance = v.getLease().getMileageAllowance();
            if (v.getFuel().getType() == 1) {
                v.getFuel();
                co2= Fuel.CO2EMITTED_GASCOMBUSTION;
            } else {
                v.getFuel();
                co2 = Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION;
            }
            double dueatsigning = v.getLease().getDueAtSigning();
            ...
            v.setCO2Emission(computeCO2(numofMonth, usage, mileageAllowance, co2));
            v.setFuelCost(computeFuelCost(numofMonth, usage, mileageAllowance, fuelprice));
            v.setTotalCost(computeLeaseCost(dueatsigning, numofMonth, monthlycost) + computeFuelCost(numofMonth, usage, mileageAllowance, fuelprice)+ charger);
       }
    }
